...
async function setQueue(steps = [], meta = {}) {
	// Store ONLY the queue. No running flag.
	await mem.set({
		pendingQueue: steps,
...
async function actOpenInbox() {
  ...
  if (link) {
    clickAndNavigate(link.el || link); // depending on your helper return shape
    await speakNav("Opening inbox.", { mode: "say" });
...
export async function resumePendingQueue() {
	...
	// Only in-page mutex. Never persist "running" state in Memory.
	...
	try {
		...
		// small delay helps Canvas SPA settle DOM/nav
		await new Promise((r) => setTimeout(r, 250));

		// ---------------------------------------------------------------------
		// ✅ SPECIAL INTERNAL STEP (non-router intent)
		// ---------------------------------------------------------------------
		...
		// ---------------------------------------------------------------------
		// ✅ NAV-SAFE QUEUE ADVANCE
		// If the step is a navigation step, PRE-SHIFT the queue BEFORE running it.
		// Otherwise Canvas navigation can interrupt execution and the same step
		// repeats forever on the next page.
		// ---------------------------------------------------------------------
		const isNav = typeof NAV_INTENTS !== "undefined" && NAV_INTENTS.has(step.intent);

		if (isNav) {
			// Pre-advance queue so we never get stuck repeating the first step.
			await shiftQueue();

			// Tiny yield lets the memory write flush before navigation kicks in.
			await new Promise((r) => setTimeout(r, 50));

			// Run the navigation step (page may unload; that's fine now).
			// We intentionally don't post-shift here (already shifted).
			return await runAction(step.intent, step.slots || {});
		}

		// Non-navigation steps: run first, shift only on success.
		const res = await runAction(step.intent, step.slots || {});
		if (res?.ok) await shiftQueue();
		return res;
	} finally {
...
export async function handleUtterance(utterance, nluDetect) {
	...
	// -------------------------------------------------------------------------
	// ✅ 23.05) New command hygiene: cancel stale queue/plan (prevents ghost steps)
	// -------------------------------------------------------------------------
	const uLow = String(cleaned || "").toLowerCase().trim();

	// If user is not responding to a follow-up mode, treat this as a fresh command.
	...
	if (!isContinuation) {
	// Wipe any old queued stuff so it can't fire "one step late"
	await clearQueue();

	// Also wipe any chain plan resume payload if you want extra safety:
// Best-effort: prevent old chain from hijacking new commands
	try {
	const st = await mem.get();
	...
	if (ctx0?.expectingFollowUp?.kind === "DUE_PICK") {
		...
		if (Date.now() - (ctx0.expectingFollowUp.createdAt || 0) > 30000) {
			...
			if (/\b(this week|week|do this week|due this week)\b/.test(u)) {
				...
				return {
					intent: intents.DUE_THIS_WEEK,
					result: { ...r, confidence: 0.9, reason: "follow-up due this week" },
				};
			}
	...
	if (ctx0?.expectingChoice) {
		...
		const looksLikeNewCommand =
			/^open\b/.test(u) ||
			/\b(dashboard|home|courses|assignments|grades|modules|quizzes|files|go back|back|help|read page|next section|repeat)\b/.test(u) ||
	...
	// -------------------------------------------------------------------------
	// 23.4) Single-utterance "section from course" support without 'and then'
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// 23.5) Dynamic chained commands — compile each chunk deterministically
	// -------------------------------------------------------------------------
	...
	if (chunks.length >= 2) {
		...
		for (const ch of chunks) {
			const s = await compileChunkToSteps(ch, detector, baseCtx);
			if (Array.isArray(s) && s.length) steps.push(...s);
		}
	...
	if (compoundB?.courseNum) {
		...
		return {
			intent: "OPEN_ASSIGNMENT_DUE_IN_COURSE",
			result: { ...r, confidence: 0.95, reason: "compound pre-NLU" },
		};
	}
	...
	// If unknown and low confidence, upgrade to QA_GENERAL (more useful)
	...
	// If still low confidence for non-chat intents, ask user to clarify
