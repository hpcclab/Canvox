...
async function actSubmitAssignment() {
  ...
  if (!btn) {
    speak(
      "I can't submit yet. Please upload your file first, " +
      "then say submit assignment again."
    );
...
const SUM_STOPWORDS = new Set([
	"the","a","an","and","or","but","if","then","than","so","to","of","in","on","for","with","at","by","from",
	"is","are","was","were","be","been","being","it","this","that","these","those","as","i","you","we","they",
	"he","she","them","his","her","our","your","my","me","us","do","does","did","not","no","yes","can","could",
	"should","would","will","just","about","into","over","under","up","down","out","very","more","most","much",
...
function splitSummaryUnits(text) {
	...
	for (const unit of [...lines, ...sents]) {
		const key = unit.toLowerCase().replace(/[^a-z0-9 ]/g, "").slice(0, 80);
		if (seen.has(key)) continue;
		seen.add(key);
		out.push(unit);
...
function tokenSetForSummary(u) {
	const words = String(u || "").toLowerCase().match(/[a-z0-9]{2,}/g) || [];
	const set = new Set();
...
function shortenUnitForSummary(u) {
	let line = String(u || "").replace(/\s+/g, " ").trim();
	if (!line) return "";
...
function buildSmartSummary(fullText, { maxSentences = 4, maxChars = 520 } = {}) {
	...
	function bonus(u) {
		...
		if (/\b(\d{1,2}:\d{2}\s*(am|pm)|\d{1,2}\/\d{1,2}\/\d{2,4}|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b/i.test(u)) b += 1.5;

		// Penalize rubric/ratings boilerplate.
		if (/\b(view rubric|ratings|full marks|poor quality|medium quality|above average|criteria|participation|craftsmanship)\b/i.test(u)) b -= 8;
...
async function actSummarizePage() {
	...
	if (!fullText || fullText.trim().length < 60) {
		await speak("I couldn't find enough text to summarize here. Try scrolling a bit or expanding details, then say summarize again.", { mode: "say" });
		...
		if (announcementHandled.intent === intents.QA_GENERAL) {
			out = {
				...
				result: {
					...
					confidence:
						typeof announcementHandled?.result?.confidence === "number"
							? announcementHandled.result.confidence
							: 0.35,
					reason:
						announcementHandled?.result?.reason || "auto-upgrade: unknown->QA_GENERAL (default)",
				},
			};
		}
...
async function actGeneralQA(utterance) {
	...
	if (/\b(hi|hello|hey|how are you|how’s it going|whats up|what's up)\b/.test(u)) {
		...
		await speak(
			"I’m doing good. Want due today, due this week, or open a course?",
			{ mode: "say" }
		);
...
function collectAnnouncementRows() {
	...
	const rowNodes = Array.from(
		...
		return /\bposted on\b/.test(t) || /\ball sections\b/.test(t) || /\/announcements\//.test(String(row.innerHTML || ""));
	});
	...
	const pushItem = (node, linkNode = null, forcedRow = null, forcedTitle = "") => {
		...
		const hasUnreadCueText = /\bunread\b/i.test(rowText) || /\bunread\b/i.test(rowTitle) || /\bunread\b/i.test(nodeTitle);
		const hasUnreadCueClass = /\bunread\b/i.test(rowClass) || /\bunread\b/i.test(nodeClass);
		const isUnread = /^unread,\s*/i.test(raw) || hasUnreadBadge || hasUnreadCueText || hasUnreadCueClass || hasUnreadGreenDot(row);
...
async function maybeHandleAnnouncementAssist(utterance) {
	...
	if (isUnreadAnnouncementTitlesCommand(u)) {
		...
		await speak(`You have ${unread.length} unread announcements.${moreLine} ${titlesLine}`, { raw: true, mode: "read" });
	...
	if (titleQuery) {
		...
		if (!target) {
			await speak("I couldn't find an announcement with that title. Please say it again or use announcement number.", { mode: "say" });
...
function getComposeDialog() {
	return Array.from(document.querySelectorAll("[role='dialog']")).find((el) => {
		const t = (el.textContent || "").toLowerCase();
...
function looksLikeDiscussionTrigger(text) {
	const lower = String(text || "").toLowerCase();
	return (
		/\b(add|create|new|start)\b.*\bdiscussion\b/.test(lower) ||
		/\bdiscussion\b.*\b(add|create|new|start)\b/.test(lower)
	);
}
...
function tryOpenCoursePicker() {
	...
	const directCourseTrigger = Array.from(
		...
		.filter(isVisible)
		.find((el) => {
			const t = `${el.textContent || ""} ${el.getAttribute?.("aria-label") || ""} ${el.id || ""} ${el.getAttribute?.("name") || ""}`.toLowerCase();
	...
	const trigger = candidates.find((el) => {
		const blob = `${el.textContent || ""} ${el.getAttribute?.("aria-label") || ""} ${el.id || ""} ${el.getAttribute?.("name") || ""}`.toLowerCase();
...
function isInboxReadCommand(text) {
	const u = String(text || "").toLowerCase();
	return (
		/\b(read|check|summarize|list|show)\b/.test(u) &&
		(/\binbox\b/.test(u) || /\bunread\b/.test(u))
	);
}

function isReadNthMessageCommand(text) {
	const u = String(text || "").toLowerCase();
	return /\b(read|open)\b/.test(u) && /\b(first|second|third|fourth|fifth|\d+)\b/.test(u) && /\b(email|message)\b/.test(u);
}
...
function extractSenderSubjectFromRow(row) {
	const senderSel = row.querySelector(
		".css-c31sii-text, [data-testid*='participants' i], [data-testid*='sender' i]"
	);
...
function cleanSubjectText(subjectText) {
	let s = String(subjectText || "").replace(/\s+/g, " ").trim();
...
function findOpenedMessagePanel(subjectHint = "") {
	const detailNodes = Array.from(
		document.querySelectorAll(".css-103zv00-view-flexItem, [data-testid*='message-detail' i], [class*='message-detail' i], main, [role='main']")
	).filter(isLikelyRightPaneElement);

	const headingNodes = Array.from(
		document.querySelectorAll("[data-testid='message-detail-header-desktop'], [data-testid*='message-detail-header' i], h1, h2, [role='heading']")
	)
		.filter(isLikelyRightPaneElement)
		.filter((h) => {
	...
	if (hintNorm && headingNodes.length) {
		...
		if (scored[0] && scored[0].s > 0) {
			const h = scored[0].h;
			let panel =
				h.closest(".css-103zv00-view-flexItem, [data-testid*='message-detail' i], [class*='message-detail' i], article, section, [role='main'], main") ||
				h.parentElement;
...
function readCurrentMessageDetail(subjectHint = "") {
	...
	let subject =
		panel.querySelector("[data-testid='message-detail-header-desktop'], [data-testid*='message-detail-header' i], h1, h2")?.textContent?.trim() ||
		"";
	...
	let sender =
		panel.querySelector("span.css-g5lcut-text, [data-testid*='author' i], [class*='author' i], [data-testid*='participants' i]")?.textContent?.trim() ||
		"";
...
function fallbackBodyFromRightPaneText(subject, sender, date) {
	...
	for (const node of nodes) {
		...
		const startIdx = (() => {
			const dateIdx = dateNorm ? lines.findIndex((l) => normalizeChoiceText(l) === dateNorm || normalizeChoiceText(l).includes(dateNorm)) : -1;
			const subjIdx = subjectNorm ? lines.findIndex((l) => normalizeChoiceText(cleanSubjectText(l)) === subjectNorm) : -1;
...
async function maybeHandleInboxAssist(utterance) {
	...
	if (isInboxReadCommand(lower)) {
		...
		const countLine = unread.length === 1
	...
	if (isReadLatestMessageCommand(lower)) {
		...
		if (detail) {
			...
			await speak(`Opening your latest message. Subject ${targetSubject} from ${targetSender}${datePart}.`, { mode: "say" });
		}
		await mem.set({ inboxAssist: { awaitingConfirm: false, awaitingPick: true } });
...
async function confirmRecipientChosen(recipientEl, typedText) {
	...
	const currentValue = String(recipientEl?.value || recipientEl?.textContent || "").trim().toLowerCase();
	const wanted = String(typedText || "").trim().toLowerCase();
...
async function trySendCompose() {
	...
	while (Date.now() - start < 2500) {
		...
		const banner = Array.from(document.querySelectorAll("[role='alert'], .ic-flash-success, .flashalert-message"))
			.find((el) => /sent|success/i.test((el.textContent || "").toLowerCase()));
...
async function handleComposeWizardStep(utterance, wizard) {
	...
	if (wizard.step === "course") {
		...
		if (wantsOptionList(u)) {
			...
			await speak("I couldn't read the course options yet. Try opening the course dropdown once, then say list my options.", {
				mode: "say",
			});
		...
		if (!options.length) {
			...
			await speak("I still can't read course options. Please open the course dropdown and say option number, or say skip course.", { mode: "say" });
	...
	if (wizard.step === "confirm") {
		...
		if (/\bsend\b/.test(lower)) {
			...
			if (!sent.ok) {
				await speak("I clicked send, but I could not confirm the message was sent. Please check the form and try send again.", { mode: "say" });
...
async function handleDiscussionWizardStep(utterance, wizard) {
	...
	if (wizard.step === "content") {
		...
		if (!ok) {
			await speak("I could not fill the topic content editor. Please click in the editor and try again.", { mode: "say" });
	...
	if (wizard.step === "require_before_reply") {
		...
		if (!cb || !setCheckboxValue(cb, yn)) {
			await speak("I could not toggle that option automatically. Please set it manually, then continue.", { mode: "say" });
	...
	if (wizard.step === "allow_liking") {
		...
		if (!cb || !setCheckboxValue(cb, yn)) {
			await speak("I could not toggle allow liking automatically. Please set it manually, then continue.", { mode: "say" });
