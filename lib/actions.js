...
function getActiveCourseId(state) {
  ...
  const fromState =
    ...
    state?.courseId ||
    getCourseIdFromHref(state?.activeCourseHref || "");
...
async function actOpenCourseByQuery(cleanedUtterance) {
	...
	if ((!q.num || q.num.length < 4) && (!q.remainder || q.remainder.length <= 6)) {
		...
		if (q.dept) {
			filtered = all.filter((o) =>
				normalizeCourseLabel(o.label).toLowerCase().includes(q.dept)
			);

			if (!filtered.length && q.dept === "cse") {
				filtered = all.filter((o) =>
					normalizeCourseLabel(o.label).toLowerCase().includes("csce")
				);
			}
		}
	...
	const courseNum =
		(best.label.match(/\b\d{4}\b/) || [null])[0] ||
		(best.href.match(/\b\d{4}\b/) || [null])[0] ||
		null;
...
function forceCommandVerb(chunk) {
  ...
  if (/^(assignments?|grades?|courses?|modules?|quizzes?|files?|announcements?|dashboard|home)\b/i.test(c)) return `open ${c}`;
  return c;
}
...
function parseOpenSectionChunk(chunk) {
  const u = String(chunk || "").toLowerCase();
  if (!/\bopen\b/.test(u) && !/^(grades?|assignments?|modules?|quizzes?|files?|announcements?|courses?|dashboard|home)\b/.test(u))
    return null;
