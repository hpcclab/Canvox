...
console.log("[CONVOX actions] build 2026-03-30T20:30 guard+no-mutation");
...
function speakCourseShort(label = "", href = "") {
	...
	const m = label.match(/\b\d{4}\b/) || href.match(/\b\d{4}\b/);

	if (m) return `course ${m[0]}`;

	// Fallback: first 3 words only
	return label.replace(/\s+/g, " ").split(" ").slice(0, 3).join(" ");
}
async function speakNav(msg, opts = {}) {
	...
	try {
		...
		console.warn("Memory.get failed:", e);
		return;
	}
...
function getActiveCourseId(state) {
	const liveMatch = window.location.pathname.match(/\/courses\/(\d+)/);
	if (liveMatch) return liveMatch[1];

	const fromState = state?.activeCourseId || state?.courseId || getCourseIdFromHref(state?.activeCourseHref || "");
...
function planUntilSatisfied(until, state) {
	...
	if (until.type === "COURSE_NUM") {
		...
		const onCoursePage = /\/courses\/\d+/.test(String(location.pathname || ""));
		return onCoursePage && want && have && want === have;
	}
...
if (!__CONVOX.actions.releasePlanMutex) {
	__CONVOX.actions.releasePlanMutex = function releasePlanMutex() {
		try {
			sessionStorage.removeItem(__PLAN_MUTEX_KEY);
		} catch {}
	};
}
...
if (!__CONVOX.actions.saveResumePlan) {
	__CONVOX.actions.saveResumePlan = function saveResumePlan(planSteps, idx, meta = {}) {
		try {
			sessionStorage.setItem(__PLAN_RESUME_KEY, JSON.stringify({ plan: planSteps, idx, meta, ts: Date.now() }));
		} catch {}
	};
}
...
if (!__CONVOX.actions.clearResumePlan) {
	__CONVOX.actions.clearResumePlan = function clearResumePlan() {
		try {
			sessionStorage.removeItem(__PLAN_RESUME_KEY);
		} catch {}
	};
}
...
if (!__CONVOX.actions.saveResumeQueue) {
	__CONVOX.actions.saveResumeQueue = function saveResumeQueue(queueSteps, meta = {}) {
		try {
			sessionStorage.setItem(__QUEUE_RESUME_KEY, JSON.stringify({ queue: queueSteps, meta, ts: Date.now() }));
		} catch {}
	};
}
...
async function resumePlanIfAny() {
	...
	try {
		...
		return await runPlan(p.steps, {
			startIdx: Number(p.i || 0),
			meta: p || {},
		});
	} finally {
...
async function actOpenCourseByQuery(cleanedUtterance) {
	...
	const courseNum = (best.label.match(/\b\d{4}\b/) || [null])[0] || (best.href.match(/\b\d{4}\b/) || [null])[0] || null;
...
function getCourseIdFromCurrentUrl() {
	const m = String(location.pathname || "").match(/\/courses\/(\d+)/i);
	return m ? m[1] : null;
}

async function refreshActiveCourseFromPage() {
	const currentCourseId = getCourseIdFromCurrentUrl();

	if (!currentCourseId) {
		await remember({
			activeCourseId: null,
			activeCourseHref: null,
			activeCourseNum: null,
		});
		return;
	}
	...
	await remember({
		...
		activeCourseHref: location.href,
	});
}
...
async function actOpenAssignmentsForCourse(courseNumRaw) {
	...
	if (/\/assignments(?:\/|$|\?)/i.test(String(location.pathname || ""))) {
		await clearPlan();
		await clearQueue();
		try {
			__CONVOX.actions.clearResumePlan?.();
		...
		return { ok: true, message: "Already on assignments" };
	}
...
async function actGoBack() {
	try {
		...
		sessionStorage.setItem("convox_manual_nav_ts", String(Date.now()));
		window.history.back();
...
async function actOpenGrades() {
	await refreshActiveCourseFromPage();

	// Hard stop: if we're already on grades, kill stale resume state and do nothing
	if (/\/grades(?:\/|$|\?)/i.test(String(location.pathname || ""))) {
		await clearPlan();
		await clearQueue();
		try {
			__CONVOX.actions.clearResumePlan?.();
		...
		await remember({
			lastIntent: intents.OPEN_GRADES,
			lastLinkText: "Grades",
			lastLinkHref: location.href,
		});

		return { ok: true, message: "Already on grades" };
	}
	...
	if (link) {
		clickAndNavigate(link.el || link);
		await speakNav("Opening grades.", { mode: "say", always: true });
		await remember({
			lastIntent: intents.OPEN_GRADES,
			lastLinkText: "Grades",
			lastLinkHref: link.href || link.el?.href || "",
		});
...
async function actOpenAssignments() {
	...
	if (courseId) {
		const scoped = findCourseScopedNavLink(courseId, "assignments", ["assignments", "assignment", "homework", "tasks"]);
...
async function actOpenModules() {
	...
	if (/\/modules(?:\/|$|\?)/i.test(String(location.pathname || ""))) {
		await clearPlan();
		await clearQueue();
		try {
			__CONVOX.actions.clearResumePlan?.();
		...
		return { ok: true, message: "Already on Modules" };
	}
	// Fallback
...
async function actOpenInbox() {
	...
	if (/\/inbox(?:\/|$|\?)/i.test(String(location.pathname || ""))) {
		await clearPlan();
		await clearQueue();
		try {
			__CONVOX.actions.clearResumePlan?.();
		...
		return { ok: true, message: "Already on inbox" };
	}
	if (hit?.el) {
		const el = hit.el;
		clickAndNavigate(el);
...
async function actOpenFiles() {
	...
	if (/\/files(?:\/|$|\?)/i.test(String(location.pathname || ""))) {
		await clearPlan();
		await clearQueue();
		try {
			__CONVOX.actions.clearResumePlan?.();
		...
		return { ok: true, message: "Already on files" };
	}
...
async function actNavigateTo(targetRaw) {
	...
	const KNOWN = [
		...
		{ re: /\b(announcements?)\b/, fn: () => actOpenAnnouncements() },
	];
...
async function actOpenQuizzes() {
	await refreshActiveCourseFromPage();
	const state = await mem.get();
	const courseId = getActiveCourseId(state);
	if (/\/quizzes(?:\/|$|\?)/i.test(String(location.pathname || ""))) {
		await clearPlan();
		await clearQueue();
		try {
			__CONVOX.actions.clearResumePlan?.();
		...
		return { ok: true, message: "Already on Quizzes" };
	}
	if (courseId) {
		const scoped = findCourseScopedNavLink(courseId, "quizzes", ["quizzes", "quiz"]);
		if (scoped) {
			clickAndNavigate(scoped);
			await speakNav("Opening quizzes.", { mode: "say", always: true });
			await remember({ lastIntent: intents.OPEN_QUIZZES, lastLinkText: "Quizzes", lastLinkHref: scoped.href });
			return { ok: true, message: "Opened: Quizzes (course-scoped)" };
		}
	}

	const link = findBestLink({ keywords: ["quizzes", "quiz"], hrefHints: ["/quizzes"] });
	if (link) {
		clickAndNavigate(link);
		await speakNav("Opening quizzes.", { mode: "say", always: true });
		await remember({ lastIntent: intents.OPEN_QUIZZES, lastLinkText: "Quizzes", lastLinkHref: link.href });
		return { ok: true, message: "Opened: Quizzes" };
	}

	await speakNav("I can't find Quizzes here. Want dashboard?", { mode: "say", always: true });
	await remember({ expectingYesNo: true, pendingAction: "OPEN_DASHBOARD" });
	return { ok: false, message: "Quizzes link not found" };
}
...
async function actSummarizePage() {
	try {
		...
		if (!text || text.trim().length < 20) {
			await speak(
				"I can only see the page title right now. Try opening the announcement details, then say summarize again.",
				{
					mode: "say",
				},
			);
		...
		await remember({
			lastIntent: intents.SUMMARIZE_PAGE,
			lastSectionId: null,
			expectingYesNo: false,
			pendingAction: null,
		});
	...
	function startOverlayRecognition() {
		...
		overlayRec.onresult = async (e) => {
			...
			if (unMatch) {
				...
				await speak(`Username set to ${unMatch[1]}. Now say set password, then your password, or say save.`, {
					mode: "say",
				});
			} else if (pwMatch) {
...
async function actLogIn(slots = {}) {
	...
	if (!transcript) {
		await speak("Say your username and password, say submit to log in, or say stay signed in to toggle that option.", {
			mode: "say",
		});
	...
	const stored = await new Promise((res) =>
		chrome.storage.local.get("convox_credentials", (d) => res(d?.convox_credentials || null)),
	);
	...
	await speak("Credentials entered but I couldn't find the login button. Please press enter to continue.", {
		mode: "say",
	});
...
function isDateLikeLine(text) {
	return /\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+\d{1,2},?\s+\d{4}\b/i.test(
		String(text || ""),
	);
}
...
function extractSenderSubjectFromRow(row) {
	...
	if (!sender) {
		sender =
			lines.find(
				(l) => !isDateLikeLine(l) && !isCourseMetaLine(l) && /,| and | & |[A-Z][a-z]+\s+[A-Z][a-z]+/.test(l),
			) ||
...
async function maybeHandleAnnouncementAssist(utterance) {
	...
	if (isRecentAnnouncementTitlesCommand(u)) {
		...
		return {
			...
			result: {
				ok: true,
				message: "Read recent announcement titles",
				confidence: 0.35,
				reason: "auto-upgrade: unknown->QA_GENERAL (default)",
			},
		};
	}
	...
	if (!rows.length && isLikelyAnnouncementDetailPage()) {
		...
		if (detail?.body) {
			...
			return {
				...
				result: {
					ok: true,
					message: "Read current announcement detail",
					confidence: 0.35,
					reason: "auto-upgrade: unknown->QA_GENERAL (default)",
				},
			};
		...
		return {
			...
			result: {
				ok: false,
				message: "Current announcement body missing",
				confidence: 0.35,
				reason: "auto-upgrade: unknown->QA_GENERAL (default)",
			},
		};
	...
	if (idx != null) {
		...
		if (sameListPath && assist.lastListedTitles?.length) {
			const ranked = assist.lastListedTitles.map((title) => findAnnouncementByTitle(rows, title)).filter(Boolean);
		...
		if (idx < 0 || idx >= base.length) {
			...
			return {
				...
				result: {
					ok: false,
					message: "Announcement index out of range",
					confidence: 0.35,
					reason: "auto-upgrade: unknown->QA_GENERAL (default)",
				},
			};
		...
		return {
			intent: intents.QA_GENERAL,
			result: { ...res, confidence: 0.35, reason: "auto-upgrade: unknown->QA_GENERAL (default)" },
		};
	}
	...
	if (titleQuery) {
		...
		if (!target) {
			...
			return {
				...
				result: {
					ok: false,
					message: "Announcement title not found",
					confidence: 0.35,
					reason: "auto-upgrade: unknown->QA_GENERAL (default)",
				},
			};
...
function looksLikeComposeTrigger(text) {
	const lower = String(text || "").toLowerCase();
	return (
		/\b(compose|write|create|start|send)\b.*\b(message|email|inbox)\b/.test(lower) || /\bnew message\b/.test(lower)
	);
}
...
function getComposeElements() {
	...
	return {
		...
		sendButton: findVisibleButtonByText(/\bsend\b/i, root) || firstMatch(["button[data-testid*='send' i]"], root),
		cancelButton:
...
function courseMenuOptions() {
	const filtered = visiblePopupItems().filter((o) => {
		...
		if (!l) return false;
		if (
			/^(back|go back|cancel|clear|close|search|type to search|no results|favorite courses|groups|all courses|courses)$/.test(
				l,
			)
		)
			return false;
...
function extractCourseOptionsFromVisibleTextBlocks() {
	...
	const options = rawLines
		.filter((line) => {
			...
			if (!l) return false;
			if (
				/^(compose message|course|select course|favorite courses|unt honors college|groups|all courses|cancel|compose|message|inbox|settings|to|subject)$/.test(
					l,
				)
			) {
				return false;
...
function getComposeCourseOptions() {
	...
	const bestSelect =
		selects
			...
			.find(
				(x) =>
					x.score >= 4 && Array.from(x.sel.options || []).filter((o) => String(o.textContent || "").trim()).length > 1,
			)?.sel || courseControl;
...
async function applyComposeCourseChoice(choice) {
	...
	if (bestSelect?.tagName === "SELECT") {
		const match = Array.from(bestSelect.options || []).find(
			(o) => normalizeCourseLabelText(o.textContent || "") === normalizeCourseLabelText(choice.label),
		);
		if (match) {
	...
	const clickableTextMatch = Array.from(
		getComposeRoot().querySelectorAll("li, div, span, button, [role='option'], [role='menuitem']"),
	)
		.filter(isVisible)
...
async function handleComposeWizardStep(utterance, wizard) {
	...
	if (wizard.step === "course") {
		...
		if (!options.length) {
			await speak(
				"I couldn't find the course options yet. Say list my options, or open the course dropdown and try again.",
				{
					mode: "say",
				},
			);
...
function isDiscussionNewPage() {
	return (
		/\/discussion_topics\/new\b/i.test(window.location.pathname) ||
		!!firstMatch(["input#discussion_title", "input[name='title']"])
	);
}
...
function getDiscussionAutosaveDialog() {
	return (
		Array.from(document.querySelectorAll("[role='dialog'], .ui-dialog, .ReactModal__Content"))
			.filter(isVisible)
			.find((el) =>
				/auto-saved content|autosaved content|load the auto-saved content/i.test(String(el.textContent || "")),
			) || null
	);
}
...
function clickDiscussionAutosaveChoice(loadSaved) {
	...
	const btn =
		Array.from(dialog.querySelectorAll("button, [role='button']")).find(
			(el) => isVisible(el) && re.test(String(el.textContent || "").trim()),
		) || null;
	if (!btn) return false;
	btn.click?.();
	return true;
...
async function startDiscussionWizard() {
	...
	if (hasDiscussionAutosaveDialog()) {
		await speak("I found auto saved content. Would you like to load the auto saved content instead? Say yes or no.", {
			mode: "say",
		});
...
async function handleDiscussionWizardStep(utterance, wizard) {
	...
	if (wizard.step === "content") {
		...
		await mem.set({ discussionWizard: wizard });
		await speak(
			"Participants must respond to the topic before viewing other replies. Do you want this option? Say yes or no.",
			{ mode: "say" },
		);
...
export async function runAction(intent, slots = {}) {
	switch (intent) {
		...
		case "OPEN_ANNOUNCEMENTS":
			return await actOpenAnnouncements();
...
export async function resumePendingQueue() {
	...
	try {
		...
		const manualNavTs = Number(sessionStorage.getItem("convox_manual_nav_ts") || 0);
		if (manualNavTs && Date.now() - manualNavTs < 10000) {
			return { ok: true, message: "Skipped queue resume after manual navigation" };
		}
		if (!q || !q.length) {
			const saved = __CONVOX.actions.loadResumeQueue();

			// Only rehydrate very fresh queues; stale ones are the source of ghost actions
			if (saved?.queue?.length && Date.now() - (saved.ts || 0) < 5000) {
				q = saved.queue;
...
if (!__CONVOX.actions.initAutoResume) {
	__CONVOX.actions.initAutoResume = function initAutoResume() {
		...
		const fire = () => {
			...
			window.__convoxResumeTimer = setTimeout(() => {
				try {
					...
					console.warn("resumePlanIfAny failed:", e);
				}
				try {
					resumePendingQueue();
				} catch (e) {
					console.warn("resumePendingQueue failed:", e);
				}
				try {
					resumePendingAnnouncementRead();
				} catch (e) {
					console.warn("resumePendingAnnouncementRead failed:", e);
				}
			}, 250);
		};
		...
		window.addEventListener("beforeunload", () => {
			try {
				sessionStorage.removeItem("__convox_plan_mutex_v1");
			} catch {}
		});
...
function hardCancelResumes() {
	...
	try {
		sessionStorage.removeItem("__convox_plan_resume_v1");
	} catch {}
	try {
		sessionStorage.removeItem("__convox_plan_mutex_v1");
	} catch {}
	try {
		sessionStorage.removeItem("convox_manual_nav_ts");
	} catch {}
	try {
		__CONVOX.actions?.clearResumeQueue?.();
	} catch {}
	try {
		sessionStorage.removeItem("__convox_queue_resume_v1");
	} catch {}
}
...
function forceCommandVerb(chunk) {
	...
	if (/^(assignments?|grades?|courses?|modules?|quizzes?|files?|announcements?|dashboard|home)\b/i.test(c))
		return `open ${c}`;
	return c;
}
...
function parseTargetSection(text = "") {
	...
	if (/\bannouncements?\b/.test(t)) return { section: "announcements" };
...
function expandCourseScopedCommand(cleaned = "") {
	...
	const mA = fixed.match(/\bopen\s+(.+?)\s+(from|in)\s+course\s+(.+)\b/i);

	// Pattern B: "from course <X> open <thing>"
	const mB = fixed.match(/\b(from|in)\s+course\s+(.+?)\s+open\s+(.+)\b/i);
...
function detectSectionIntentFromText(t) {
	...
	if (/\bannouncements?\b/.test(u)) return "OPEN_ANNOUNCEMENTS";
...
function parseOpenSectionChunk(chunk) {
	...
	if (
		!/\bopen\b/.test(u) &&
		!/^(grades?|assignments?|modules?|quizzes?|files?|announcements?|courses?|dashboard|home)\b/.test(u)
	)
		return null;
	...
	else if (sec === "OPEN_ANNOUNCEMENTS") pathNeed = "/announcements";

	return pathNeed
		? { intent: sec, slots: {}, until: { type: "PATH_INCLUDES", value: pathNeed } }
		: { intent: sec, slots: {} };
}
...
export async function handleUtterance(utterance, nluDetect) {
	...
	if (isReadRecentMessageCommand(cleaned)) {
		...
		return {
			intent: intents.READ_RECENT_MESSAGE,
			result: { ...result, confidence: 0.98, reason: "direct inbox recent message" },
		};
	}
	...
	if (ctx0?.expectingChoice) {
		...
		const looksLikeNewCommand =
			/^open\b/.test(u) ||
			/\b(dashboard|home|courses|assignments|grades|modules|quizzes|files|inbox|messages|go back|back|help|read page|next section|repeat|compose)\b/.test(
				u,
			) ||
	...
	if (special && special.length >= 2) {
		...
		await setPlan(special, meta);
		await runPlan(special, { startIdx: 0, meta });
	...
	if (chunks.length >= 2) {
		...
		if (steps.length >= 2) {
			...
			await setPlan(steps, meta);
			await runPlan(steps, { startIdx: 0, meta });
	...
	if (intent === intents.SMALL_TALK || intent === intents.QA_GENERAL || intent === intents.LOG_IN) {
...
async function actOpenAnnouncements() {
	await refreshActiveCourseFromPage();

	const state = await mem.get();
	const courseId = getActiveCourseId(state);
	if (/\/announcements(?:\/|$|\?)/i.test(String(location.pathname || ""))) {
		await clearPlan();
		await clearQueue();
		try {
			__CONVOX.actions.clearResumePlan?.();
		...
		return { ok: true, message: "Already on announcements" };
	}
	if (courseId) {
		const scoped = findCourseScopedNavLink(courseId, "announcements", ["announcements", "announcement"]);
		if (scoped) {
			clickAndNavigate(scoped);
			await speakNav("Opening announcements.", { mode: "say", always: true });
			await remember({
				lastIntent: "OPEN_ANNOUNCEMENTS",
				lastLinkText: "Announcements",
				lastLinkHref: scoped.href,
			});
			return { ok: true, message: "Opened: Announcements (course-scoped)" };
		}
	}

	const link = findBestLink({
		keywords: ["announcements", "announcement"],
		hrefHints: ["/announcements"],
	});

	if (link) {
		clickAndNavigate(link.el || link);
		await speakNav("Opening announcements.", { mode: "say", always: true });
		return { ok: true, message: "Opened: Announcements" };
	}

	await speakNav("I can't find announcements here.", { mode: "say", always: true });
	return { ok: false, message: "Announcements link not found" };
}
