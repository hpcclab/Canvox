...
async function actComposeMessage() {
	if (!/\/conversations\b/i.test(window.location.pathname)) {
		await speak("Opening inbox first.", { mode: "say" });
		await setPlan([{ intent: "OPEN_INBOX" }, { intent: "COMPOSE_MESSAGE" }], { kind: "COMPOSE_MESSAGE" });
		return await actOpenInbox();
	}

	const opened = await ensureComposerOpen();
	if (!opened.ok) {
		await speak("I couldn't open the compose message popup. Open it manually, then say compose a message again.", { mode: "say" });
		return { ok: false, message: opened.message || "Composer open failed" };
	}

	await mem.set({
		composeDraft: { course: "", recipient: "", subject: "", body: "" },
		composeFlow: null,
	});
	return await askForCourseSelection(opened.root);
}
...
function writeDiscussionContent(text) {
	return writeField(getDiscussionContentEditable(), text);
}
...
async function startDiscussionWizard() {
	...
	await speak("What would you like as the discussion title?", { mode: "say" });
...
async function handleDiscussionWizardStep(utterance, wizard) {
	...
	if (wizard.step === "title") {
		...
		await speak("Now say the discussion content.", { mode: "say" });
	...
	if (wizard.step === "content") {
		...
		wizard.step = "confirm";
		await mem.set({ discussionWizard: wizard });
		await speak("Say submit to post it, or cancel.", { mode: "say" });
		return { ok: true, message: "Discussion content set" };
	}

	if (wizard.step === "confirm") {
		if (/\b(submit|post|save)\b/.test(lower)) {
		...
		await speak("Say submit to post it, or cancel.", { mode: "say" });
		return { ok: false, message: "Awaiting discussion confirmation" };
	}
