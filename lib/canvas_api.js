...
export function normalizeCanvasItem(x) {
	...
	const title = x?.plannable?.title || pl?.name || pl?.title || x?.title || "(Untitled)";

	const dueAt = pl?.due_at || x?.plannable_date || x?.todo_date || x?.due_at || null;

	const courseId = x?.course_id || pl?.course_id || x?.context_id || null;

	const courseName = x?.course_name || x?.context_name || pl?.course_name || null;

	const htmlUrl = pl?.html_url || x?.html_url || x?.plannable?.html_url || null;

	const points = pl?.points_possible ?? x?.points_possible ?? null;

	const submitted = pl?.has_submitted_submissions ?? x?.has_submitted_submissions ?? null;

	const type = x?.plannable_type || pl?.submission_types ? "assignment" : x?.type || "item";
