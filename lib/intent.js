...
const RE = {
	...
	openDashboard: new RegExp(`\\b(${anySyn("open")})\\s+(my\\s+)?(${anySyn("home")})\\b|\\b(go\\s+home)\\b`),
	...
	composeMessage: new RegExp(`\\b(${anySyn("compose")})\\b.*\\b(${anySyn("message")})\\b`),
...
export async function detectIntent(rawUtterance = "", context = {}) {
	...
	//compose message
	if (RE.composeMessage.test(u)) return scored(intents.COMPOSE_MESSAGE, 0.96, {}, "rule: composeMessage");
	if (RE.addDiscussion.test(u)) return scored(intents.ADD_DISCUSSION, 0.97, {}, "rule: addDiscussion");
